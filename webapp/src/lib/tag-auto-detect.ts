/**
 * Client-side tag auto-detection — mirrors convex/tagAutoDetectCore.ts.
 * Keep detection rules in sync when changing either file.
...
function detectRouteStyle(legs: RouteLeg[], totalKm: number, out: TagSuggestion[]) {
  ...
  if (legs.length === 1) {
    addSuggestion(out, 'route_style.linear', 0.45, 'Single leg with distinct start and end', 'geometry.linear');
  }
}
...
function detectDifficulty(stats: RouteStats, out: TagSuggestion[]) {
  ...
  if (itraScore < 15) {
    ...
    addSuggestion(out, 'difficulty.hard', 0.7, `ITRA effort ~${itraScore.toFixed(0)} (hard)`, 'stats.grade_hard');
...
const POI_TYPE_TO_TAGS: Record<PoiType, { slug: string; confidence: number; reason: string }[]> = {
  ...
  facility: [{ slug: 'facilities.visitor_centre', confidence: 0.8, reason: 'Visitor facility on route' }],
  viewpoint: [{ slug: 'features.great_views', confidence: 0.55, reason: 'Viewpoint marked on route' }],
  nature_reserve: [{ slug: 'features.nature_reserve', confidence: 0.85, reason: 'Nature reserve on route' }],
  wildlife: [{ slug: 'features.wildlife', confidence: 0.5, reason: 'Wildlife POI on route' }],
  landmark: [{ slug: 'features.historic_site', confidence: 0.55, reason: 'Landmark on route' }],
  hazard: [{ slug: 'hazards.steep_drops', confidence: 0.4, reason: 'Hazard marked on route' }],
  navigation: [],
  accommodation: [],
};

function detectPois(poiTypes: PoiType[], out: TagSuggestion[]) {
...
function finalizeSuggestions(suggestions: TagSuggestion[], minConfidence: number): TagSuggestion[] {
...
/** Instant suggestions in the planner (slug-only; resolve tagIds via Convex when saving). */
export function suggestTagsForRoute(args: {
  legs: RouteLeg[];
  stats?: RouteStats;
  poiTypes?: PoiType[];
  ...
  const stats: RouteStats = {
    distanceKm: args.stats?.distanceKm && args.stats.distanceKm > 0 ? args.stats.distanceKm : computedKm,
